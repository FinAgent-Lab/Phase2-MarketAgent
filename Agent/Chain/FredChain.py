"""
FRED 기준금리 데이터와 S&P 500 섹터 데이터를 분석하여 
향후 유망한 섹터를 예측하는 모델
"""
...
from Agent.Prompt.FredPrompt import ANALYSIS_PROMPT, BACKTEST_PROMPT, FEEDBACK_PROMPT
from Agent.Prompt.FredPrompt import first_output_parser, backtest_output_parser, feedback_output_parser
...
# 환경 변수 로드
class FederalFundsRateAnalyzer:
    """FRED API를 사용한 기준금리 데이터 분석 클래스"""
    
    def __init__(self):
        self.fred = Fred(api_key=os.getenv('FRED_API_KEY'))
    
    def get_rate_changes(self) -> pd.DataFrame:
        """기준금리 목표 범위의 변화 시점만 추출"""
        target_upper = self.fred.get_series('DFEDTARU')  # 상한선
        target_lower = self.fred.get_series('DFEDTARL')  # 하한선
        
        # 변화가 있는 시점만 필터링
        changes_mask = (target_upper.diff() != 0) | (target_lower.diff() != 0)
        
        return pd.DataFrame({
            '하한선': target_lower[changes_mask],
            '상한선': target_upper[changes_mask]
        })
    
    def get_rate_data_by_period(self, start_date: str, end_date: str) -> pd.DataFrame:
        """특정 기간의 기준금리 데이터 조회"""
        rate_changes = self.get_rate_changes()
        return rate_changes.loc[start_date:end_date]




class FredRateSectorAnalyzer:
    """섹터 분석 및 추천 시스템
    ...
    def load_data(self):
        ...
        if self._sector_data is None or self._rate_data is None:
            print(f"📊 데이터 로딩 중... (start: {self.start_date}, end: {self.end_date})")
            
            # 금리 데이터 수집
            rate_analyzer = FederalFundsRateAnalyzer()
            self._rate_data = rate_analyzer.get_rate_data_by_period(self.start_date, self.end_date)
    ...
    def get_filtered_data_before_three_months(self):
        ...
        rate_data = self.rate_data.loc[:self.three_months_ago]
        return sector_data, rate_data
    ...
    def first_analysis(self):
        ...
        # 3개월 전까지의 데이터로 분석
        sector_data, rate_data = self.get_filtered_data_before_three_months()
        
        chain = ANALYSIS_PROMPT | self.llm | first_output_parser
        first_result = chain.invoke({
            "before_rate_data": rate_data,
