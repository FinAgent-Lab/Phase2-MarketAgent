...
def recommend_sectors_chain(llm, macro_data, sector_data, prompt):
    ...
    result = chain.invoke({"macro_data": macro_data, "sector_data": sector_data})
...
def summarize_data_chain(llm, data_package, prompt):
    ...
    result = chain.invoke({"data_package": data_package})
    return result
...
def feedback_analysis_chain(llm, summary_data, evaluation_response, prompt):
    ...
    result = chain.invoke(
        {"summary_data": summary_data, "evaluation_response": evaluation_response}
    )
    return result
