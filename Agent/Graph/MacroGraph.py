...
def summarize_recommend_data(state: GraphState):
    ...
    result = summarize_data_chain(
        llm=model, data_package=data_merge, prompt=SUMMARY_PROMPT
    )
...
def final_summary(state: GraphState):
    ...
    final_result = final_summary_chain(
        llm=model, result=result_package, prompt=FINAL_SUMMARY_PROMPT
    )

    return {"FinalSummary": final_result}
...
workflow.add_conditional_edges(
    "run_evaluation",
    should_feedback,
    {"feedback": "feedback_analysis", "end": "final_summary"},
)
